const { each, iff, prop } = require('rvl-pipe')
...
const startListening = () => ctx => {
  const port = process.env.PORT || '3001'
  ctx.app.listen(port, () => console.log(`App listening on port ${port}!`))
  return ctx
}

const complement = fn => ctx => !fn(ctx)

const createHandler = handlers => ctx => {
  handlers.forEach(handler => {
    ctx.app[handler.method](handler.path, ...[...handler.middlewares || [], wrap(handler.fn)])
  })

  return ctx
}

module.exports.createServer = (initializing, handlers) => each(
  initializing,
  createApp(),
  createHandler(handlers),
  iff(
    complement(prop('noStart')),
    startListening()
  )
)
