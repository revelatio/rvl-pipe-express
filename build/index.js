...
var withContext = function (ctx) { return function (__, res, next) {
    res.ctx = __assign({}, ctx);
    next();
}; };
var createApp = function () { return function (ctx) {
    var app = express();
    app.use(bodyParser.json());
    app.use(cookieParser());
    app.use(withContext(ctx));
    return Promise.resolve(__assign(__assign({}, ctx), { app: app, express: express }));
}; };
var startListening = function () { return function (ctx) {
    var port = process.env.PORT || '3000';
    return Promise.resolve(__assign(__assign({}, ctx), { server: ctx.app.listen(port) }));
}; };
var stopListening = function () { return function (ctx) {
    ctx.server.close();
    ...
    var addHandlers = function (router, handlers) {
        handlers.forEach(function (handler) {
            if (handler.handlers) {
                ...
                router.use.apply(router, __spreadArrays([handler.path], __spreadArrays((handler.ctxMiddlewares || []).map(function (f) { return f(ctx); }), (handler.middlewares || []), [
                    pathRouter
                ])));
            }
            else if (handler.method && handler.fn) {
                router[handler.method].apply(router, __spreadArrays([handler.path], __spreadArrays((handler.ctxMiddlewares || []).map(function (f) { return f(ctx); }), (handler.middlewares || []), [
                    wrap(handler.fn)
                ])));
            }
        });
...
var createServer = function (handlers) {
    return rvlPipe.each(createApp(), createHandler(handlers));
};
...
var validateIO = function (errorType, propName) { return function (validator) { return function (ctx) {
    ...
    if (!valid) {
        return Promise.reject(new rvlPipe.ContextError(errorType, ctx));
    }
    return Promise.resolve(ctx);
}; }; };
var validateRequest = validateIO('InvalidRequest', 'body');
